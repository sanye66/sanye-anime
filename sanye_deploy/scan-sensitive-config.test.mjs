...
test('first-party mode scans application and sanye JARs and reports skipped dependency boundary', async t => {
  ...
  run(root, 'jar', ['--create', '--no-manifest', '--file', 'output/BOOT-INF/lib/sanye-common.jar', '-C', 'output/BOOT-INF/classes', 'application.yml'])
  await writeFile(path.join(root, 'output', 'BOOT-INF', 'lib', 'third-party.jar'), 'unreadable third-party fixture')
  run(root, 'jar', ['--create', '--no-manifest', '--file', 'output/app.jar', '-C', 'output', 'BOOT-INF'])
...
test('scans ignored logs and JAR entry content when explicitly selected', async t => {
  ...
  run(root, 'jar', ['--create', '--no-manifest', '--file', 'output/app.jar', '-C', 'output', 'application.yml'])
  run(root, 'jar', ['--create', '--no-manifest', '--file', 'output/outer.jar', '-C', 'output', 'app.jar'])
