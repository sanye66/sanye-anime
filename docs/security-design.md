...
当前严格配置校验与日志防护已实现，T-R-05 的凭据复用/轮换确认仍待环境。本次仅重跑安全相关测试，未重新扫描所有历史、日志、产物或验证真实凭据；具体证据范围见[当前审计](./current-status-audit.md)。
...
业务核心与管理应用分别通过 `EnvironmentPostProcessor` 在配置加载后、连接和 Bean 创建前执行校验。`SANYE_ENV` 缺失时按严格 `test` 规则处理；仅显式 `local` 允许本地开发凭据，禁止与部署 profile 混用。JWT、内部调用令牌至少 32 字符；非本地数据库、Redis 及已启用外部组件凭据至少 12 字符，拒绝空值、未解析引用、已知开发默认和占位值。具体变量以 [环境配置](./environment-config.md) 为准。

校验失败只输出属性名，不传播可能包含原值的占位解析异常。Actuator 环境和配置属性端点强制不显示原值。管理 HTTP 工具不记录 URL、参数、正文或原始异常；HTTPS 使用默认证书与主机名校验，参数放入 POST 请求体。管理端默认关闭热重载和 Druid 控制台，日志级别为 INFO。

`scan-sensitive-config.mjs` 只报告路径、行号、规则和分类，读取失败或未决候选使退出码非零；不得按测试目录整体放行、重写历史或删除旧日志关闭门禁。`first-party` 产物模式仅排除报告列出的第三方依赖 JAR，不代表完整供应链扫描。规则命中不能证明凭据是否真实有效。外部环境若曾复用公开开发值，维护人须通过受控渠道轮换数据库、Redis、JWT、管理/内部调用令牌及中间件账号，并撤销旧会话；本轮未操作真实凭据。

更新记录：2026-09-10，v1.2，新增 T-R-05 启动、日志、HTTPS 与审计规则；证据见 [任务记录](./development-tasks.md)。
