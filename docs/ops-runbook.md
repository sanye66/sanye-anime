...
当前独立备份恢复按 [独立恢复说明](./backup-recovery.md) 执行，任务证据统一见 T-R-04。第 9 节的旧备份命令仅保留历史背景；新入口要求显式配置两套数据库、MinIO 和本地文件目录，拒绝覆盖现有目标，不自动删除历史备份。

更新记录：2026-09-10，v0.10，同步独立恢复入口与非覆盖边界；依据 `recovery.mjs`、专项测试及开发任务清单。

## 当前核对（2026-09-10）

排障前先确认实际监听、配置与目标实例；本次常用前端及网关端口未监听，不能用历史健康报告替代现场证据。Mock CAS 需独立启动且与前端登录地址对齐，见[环境矩阵](./environment-config.md)。恢复使用 V2 显式源/目标清单，发布实例核验见[CI/CD](./ci-cd.md)。

更新记录：2026-09-10，v0.11，按当前代码与进度校正本文事实或证据范围；依据上述源码、任务与审计引用。
...
### 9.2 备份与恢复（2026-08-19 历史记录）

以下是旧脚本的历史结果，不适用于当前操作；现行流程与本轮实测见 [独立恢复说明](./backup-recovery.md) 和开发任务清单 T-R-04。
