...
更新记录：2026-09-17，v0.16，外部搜索候选增加 `alternativeSourceUrls: string[]`，默认空数组、最多三个。按同名作品合并结果时仍保留主要 `sourceUrl` 和分类优先级，但不丢弃其他不同的来源页。客户端将其传入直接观看页面，各来源继续通过现有白名单预览接口解析；仅对同名、单集电影合并播放线路，电视剧不跨来源合并集数。
