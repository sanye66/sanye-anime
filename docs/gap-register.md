...
| GAP-002 | CI/CD | CI 工作流已落地（T-B-02 完成：[ci.yml](../.github/workflows/ci.yml) 文档、前端、双后端、浏览器安全回归、依赖扫描、密钥扫描六作业，本地等价门禁已验证）；镜像构建、注册表和分支保护仍待启用 | `in-progress` | 启用分支保护并在部署环境接入镜像发布 | 拉取请求检查通过，失败检查会阻止进入测试或发布分支 | P0 |
...
| 2026-08-26 | v0.18 | 同步 CI 为六作业，管理后端执行测试并增加自包含浏览器回归；外部镜像发布与分支保护状态不变 | `.github/workflows/ci.yml`、本地等价门禁 |
