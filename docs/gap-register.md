...
更新记录：2026-09-10，v0.22，T-R-06 已补齐候选镜像与核验入口并通过本地专项；固定候选、真实注册表、环境和分支保护仍待环境，依据[任务证据](./development-tasks.md#t-r-06-执行与证据2026-09-10)。

与管理后端真实运行、前端联调和交付相关的具体执行项，统一维护在 [开发待办事项](./development-todo.md)。本表保留差距状态和阶段出口标准，待办文档负责拆分具体工作和验收动作。

更新记录：2026-09-10，v0.21，重新复评 GAP-010，最终状态以开发任务清单 T-R-04 的全部问题修正与复验为准，不复用上一轮遗漏问题时的完成结论；生产配置和发布保持原范围。

## 当前核对（2026-09-10）

当前正常使用仍受集成环境、真实身份/模型、外部媒体及发布证据阻塞，详见[当前审计](./current-status-audit.md)。`verified` 只表示对应出口在已登记环境通过，不代表今天的目标实例正在运行。

更新记录：2026-09-10，v0.23，按当前代码与进度校正本文事实或证据范围；依据上述源码、任务与审计引用。
...
| GAP-002 | CI/CD | 六作业 CI 已落地；T-R-06 已实现候选镜像、配置指纹和实例核验，本地专项通过；未运行完整固定候选流水线，注册表推送待环境，远端分支保护查询为 HTTP 401 | `blocked-external` | 确认候选与环境，取得远端规则读取权限并验证强制门禁、镜像推送及实例对应关系 | 拉取请求检查通过，失败检查会阻止进入测试或发布分支 | P0 |
...
| GAP-004 | 内容数据 | 已有本地目录、历史导入及隔离合成数据；尚无满足来源授权及规模出口标准的目标环境数据集 | `blocked-external` | 获取授权来源并准备小型种子数据集 | 20 至 50 部测试作品具备来源、状态、图片和排期证据 | P0 |
...
| GAP-009 | 版本兼容性 | RuoYi 管理后端历史 JDK 17 构建、PG/Quartz 适配、真实启动/登录/权限/Quartz/前端联调已验证；搜索 ES 索引命中、文件 MinIO 对象/元数据导入和 RabbitMQ 事件发布/消费已在受控环境验证；T-R-03 已完成 XXL-JOB 索引重建与 RuoYi 管理整合的受控验收 | `in-progress` | 按 Java 21 当前基线在目标环境联合复测登录、搜索、存储及两类任务 | 最小启动、构建、登录、搜索、存储和任务执行全部通过 | P0 |
| GAP-010 | 备份恢复 | T-R-04 全部已发现问题已修正，最终代码通过两库、MinIO 和本地文件恢复及应用验收；100 项表/序列、15 项专项和 31 项真实演练证据见 [开发任务清单](./development-tasks.md) 最终复验记录 | `verified` | 后续按 T-R-05 至 T-R-07 在已确认发布环境配置凭据并复用恢复流程 | 干净测试环境能够恢复数据并完成结果校验 | P0 |
...
- `sanye_server` 已具备 Spring Boot 启动类、Maven 入口、网关与业务服务、PostgreSQL/Redis 持久化和本地联调脚本；ES 搜索、MinIO 文件对象的本地数据面已验证，RabbitMQ 业务事件与 XXL-JOB 索引任务已有 T-R-02/03 受控验收，目标发布环境仍待验证。
- `sanye_admin_server` 已完成 RuoYi 模块接入、Java 17 Maven 构建和 PostgreSQL/Quartz 初步适配；构建证据为 2026-09-03 `BUILD SUCCESS`。
...
2026-09-09，v0.19：GAP-009 中 RabbitMQ 真实事件缺口已由 T-R-02 关闭；XXL-JOB 与目标部署验证仍保留。
