package com.sanye.admin.framework.aspectj;
...
import com.sanye.admin.common.core.domain.BaseEntity;
...
import com.sanye.admin.common.core.text.Convert;
import com.sanye.admin.common.utils.SecurityUtils;
import com.sanye.admin.common.utils.StringUtils;
import com.sanye.admin.framework.security.context.PermissionContextHolder;
        ...
        for (SysRole role : user.getRoles())
        {
            ...
            else if (Constants.Dept.DATA_SCOPE_CUSTOM.equals(dataScope))
            {
                if (scopeCustomIds.size() > 1)
                {
                    ...
                    sqlString.append(StringUtils.format(" OR {}.{} IN ( SELECT dept_id FROM sanye_sys_role_dept WHERE role_id in ({}) ) ", deptAlias, deptField, String.join(",", scopeCustomIds)));
                }
                else
                {
                    sqlString.append(StringUtils.format(" OR {}.{} IN ( SELECT dept_id FROM sanye_sys_role_dept WHERE role_id = {} ) ", deptAlias, deptField, role.getRoleId()));
                }
            }
            ...
            {
                sqlString.append(StringUtils.format(" OR {}.{} IN ( SELECT dept_id FROM sanye_sys_dept WHERE dept_id = {} or (',' || ancestors || ',') LIKE ('%,' || {} || ',%') )", deptAlias, deptField, user.getDeptId(), user.getDeptId()));
            }
