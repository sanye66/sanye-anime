...
function topbar() {
  ...
  return `
    <header class="prototype-topbar">
      ...
      ${productSwitcher()}
      <div class="prototype-meta"><button class="theme-toggle" data-action="toggle-theme" aria-label="切换到${nextTheme === 'light' ? '亮色' : '暗色'}主题" aria-pressed="${state.theme === 'light'}" title="切换到${nextTheme === 'light' ? '亮色' : '暗色'}主题"><span>${icon(nextTheme === 'light' ? 'sun' : 'moon')}</span></button><span class="live-dot"></span>页面原型 · 可交互预览</div>
    </header>
  `;
}
...
function clientHeader() {
  return `
    <div class="client-header">
      <div><span class="section-kicker">${state.page === 'home' ? '星期三 · 9 月 18 日' : 'sanye_anime / 用户空间'}</span><h1>${state.page === 'home' ? '晚上好，林默。' : clientPageTitle()}</h1></div>
...
function searchPage() {
  ...
  const results = query ? animeCatalog.filter((anime) => `${anime.title}${anime.originalTitle}${anime.tags.join('')}`.toLowerCase().includes(query)) : animeCatalog;
  return `
    <div class="page-content client-page">
      ${clientHeader()}
      <section class="search-panel"><div class="large-search"><span>${icon('search')}</span><input id="search-input" value="${escapeHtml(state.searchQuery)}" placeholder="搜索作品、角色、类型或关键词" autofocus /><kbd>⌘ K</kbd></div><div class="filter-row"><button class="filter-chip is-active">全部</button><button class="filter-chip">电视动画</button><button class="filter-chip">剧场版</button><button class="filter-chip">科幻</button><button class="filter-chip">治愈</button><span class="result-count">${results.length} 部作品</span></div></section>
...
function aiPage() {
  ...
  return `
    <div class="page-content client-page ai-page-content">
      ${clientHeader()}
      <section class="ai-workspace"><aside class="conversation-panel"><div class="conversation-heading"><div><span class="section-kicker">你的空间</span><h2>会话</h2></div><button class="icon-only dark-icon" data-action="new-chat" title="新建会话">＋</button></div><button class="new-chat-button" data-action="new-chat">＋ 新建会话</button><div class="conversation-group"><span class="group-label">今天</span><button class="conversation-item is-active"><span class="conversation-icon">✦</span><span><strong>星海回声的角色关系</strong><small>刚刚 · 3 条消息</small></span><span class="conversation-more">···</span></button><button class="conversation-item" data-action="load-chat" data-chat="afterglow"><span class="conversation-icon">◌</span><span><strong>找一部温柔的夏日动画</strong><small>昨天 · 7 条消息</small></span></button></div><div class="conversation-group"><span class="group-label">更早</span><button class="conversation-item"><span class="conversation-icon">◌</span><span><strong>推荐悬疑剧场版</strong><small>9 月 12 日 · 5 条消息</small></span></button></div><div class="conversation-bottom"><span class="ai-usage"><i></i><span><strong>今日 AI 额度</strong><small>还剩 24 / 30 次</small></span></span><button class="text-link">查看规则</button></div></aside><main class="chat-panel"><div class="chat-top"><div><span class="section-kicker">当前会话</span><h2>星海回声的角色关系</h2></div><div class="chat-top-actions"><button class="icon-only" title="搜索会话">⌕</button><button class="icon-only" title="更多操作">···</button></div></div><div class="chat-messages">${state.messages.map((message) => `<div class="message-row ${message.role}"><span class="message-avatar ${message.role === 'assistant' ? 'assistant-avatar' : 'user-avatar'}">${message.role === 'assistant' ? '✦' : '林'}</span><div class="message-bubble"><span class="message-name">${message.role === 'assistant' ? '三叶助手' : '林默'}</span><p>${escapeHtml(message.text)}</p>${message.recommendation ? `<button class="message-recommendation" data-anime="${message.recommendation.id}">${coverArt(message.recommendation, 'message-cover')}<span><strong>${message.recommendation.title}</strong><small>与你的问题有关 · ★ ${message.recommendation.score}</small></span><span>${icon('arrow')}</span></button>` : ''}</div></div>`).join('')}${state.pendingAnswer ? `<div class="message-row assistant"><span class="message-avatar assistant-avatar">✦</span><div class="message-bubble pending-bubble"><span class="message-name">三叶助手</span><div class="typing-dots"><i></i><i></i><i></i></div></div></div>` : ''}</div><div class="chat-composer"><div class="composer-tools"><button class="composer-tool ${state.spoilerMode ? 'is-active' : ''}" data-action="toggle-spoiler"><span>${state.spoilerMode ? '◉' : '◎'}</span>${state.spoilerMode ? '允许剧透' : '避免剧透'}</button><button class="composer-tool">＋ 添加作品</button></div><form class="composer-form" data-action="send-ai"><input id="ai-input" placeholder="问问关于动漫的任何事..." autocomplete="off" /><button class="send-button" type="submit" title="发送问题">${icon('arrow')}</button></form><div class="suggestion-row"><button data-action="fill-ai" data-prompt="不用剧透，告诉我白石澄为什么离开地表？">不用剧透，解释一下白石澄</button><button data-action="fill-ai" data-prompt="推荐三部和《星海回声》一样有城市感的作品">推荐相似作品</button><button data-action="fill-ai" data-prompt="这部作品适合什么心情的时候看？">适合什么心情看？</button></div></div></main><aside class="context-panel"><div class="context-heading"><span class="section-kicker">当前上下文</span><button class="icon-only" title="移除上下文">×</button></div><div class="context-anime">${coverArt(anime, 'context-cover')}<strong>${anime.title}</strong><small>${anime.meta}</small><button data-anime="${anime.id}">查看详情 ${icon('arrow')}</button></div><div class="context-setting"><span class="section-kicker">回答设置</span><label><span>剧透模式</span><button class="toggle ${state.spoilerMode ? 'is-on' : ''}" data-action="toggle-spoiler"><i></i></button></label><label><span>引用作品信息</span><button class="toggle is-on"><i></i></button></label><label><span>推荐相似作品</span><button class="toggle is-on"><i></i></button></label></div><div class="context-note"><span>ⓘ</span><p>回答会优先参考已发布的作品信息。AI 生成内容仅供参考。</p></div></aside></section>
    </div>
  `;
}
...
function minePage() {
  return `
    <div class="page-content client-page">
      ${clientHeader()}
      <section class="profile-overview"><div class="profile-avatar">林</div><div><span class="section-kicker">当前账号</span><h2>林默</h2><p>从 2025 年 8 月开始探索 sanye_anime</p></div><button class="button button-outline">编辑资料</button></section>
      <section class="mine-grid"><div class="mine-block"><div class="section-heading"><div><span class="section-kicker">已收藏作品</span><h2>我的收藏 <small>2</small></h2></div><button class="text-link">查看全部 ${icon('arrow')}</button></div><div class="saved-list">${animeCatalog.filter((anime) => state.favorites.has(anime.id)).map((anime) => `<button class="saved-item" data-anime="${anime.id}">${coverArt(anime, 'saved-cover')}<span><strong>${anime.title}</strong><small>${anime.meta}</small><em>已收藏 · ★ ${anime.score}</em></span><span>${icon('arrow')}</span></button>`).join('')}</div></div><div class="mine-block"><div class="section-heading"><div><span class="section-kicker">最近浏览</span><h2>最近浏览</h2></div><button class="text-link">清空记录</button></div><div class="history-list"><button class="history-item" data-anime="skyline"><span class="history-time">今天<br><strong>20:42</strong></span><span class="history-mark cover-coral"></span><span><strong>星海回声</strong><small>作品详情 · 停留 3 分钟</small></span></button><button class="history-item" data-page="ai"><span class="history-time">昨天<br><strong>22:18</strong></span><span class="history-mark ai-history">✦</span><span><strong>找一部温柔的夏日动画</strong><small>AI 会话 · 7 条消息</small></span></button></div></div></section><section class="settings-section"><div class="section-heading"><div><span class="section-kicker">偏好设置</span><h2>偏好设置</h2></div></div><div class="preference-list"><button><span><strong>默认剧透模式</strong><small>新建 AI 会话时的默认回答方式</small></span><strong class="pref-value">避免剧透 <span>›</span></strong></button><button><span><strong>通知</strong><small>作品更新、会话结果和产品消息</small></span><strong class="pref-value">已开启 <span>›</span></strong></button><button><span><strong>隐私与数据</strong><small>管理你的收藏、历史和对话数据</small></span><strong class="pref-value">管理 <span>›</span></strong></button></div></section>
    </div>
  `;
}

function settingsPage() {
  return `<div class="page-content client-page">${clientHeader()}<section class="settings-section settings-page"><div class="section-heading"><div><span class="section-kicker">账号设置</span><h2>设置</h2></div></div><div class="preference-list"><button><span><strong>账户与登录</strong><small>当前登录设备和安全设置</small></span><strong class="pref-value">林默 <span>›</span></strong></button><button><span><strong>默认剧透模式</strong><small>控制 AI 回答中是否展示剧情信息</small></span><strong class="pref-value">避免剧透 <span>›</span></strong></button><button><span><strong>数据与隐私</strong><small>导出或删除你的个人数据</small></span><strong class="pref-value">管理 <span>›</span></strong></button></div><button class="button button-danger">退出登录</button></section></div>`;
}
...
function render() {
  ...
  if (state.product === 'client') {
    ...
    const page = state.page === 'content' ? adminContent() : state.page === 'feedback' ? adminFeedback() : state.page === 'tasks' ? adminTasks() : adminDashboard();
...
function sendAi(text) {
  ...
  window.setTimeout(() => {
    ...
    const recommendation = question.includes('推荐') || question.includes('相似') ? getAnime('paper-moon') : null;
    state.messages.push({
      role: 'assistant',
      text: state.spoilerMode ? '在避免剧透的范围内，白石澄离开地表的原因和她对信号的执着有关。更具体的解释会涉及后续集数，你可以继续观看后再问我。' : '白石澄离开地表，是因为她发现旧广播塔的信号并不是故障，而是一段被人为隐藏的求救记录。她希望在修复信号之前，先确认发出它的人是否还在等待。',
      recommendation,
    });
    render();
  }, 700);
...
document.addEventListener('click', (event) => {
  ...
  if (page) {
    if (page.startsWith('official-')) {
      ...
      state.page = page;
      render();
    } else if (state.product === 'admin' && ['dashboard', 'content', 'feedback', 'tasks'].includes(page)) {
      state.page = page;
      render();
    } else {
      openClientPage(page);
  ...
  if (action === 'show-download-message') {
    ...
    if (toast) {
      toast.textContent = '桌面安装包尚未发布，当前可直接体验静态客户端原型。';
  ...
  if (action === 'advance-feedback') {
    ...
    state.feedbackStatus[id] = current === '待处理' ? '处理中' : current === '处理中' ? '已关闭' : '待处理';
    render();
  }
});
...
document.addEventListener('input', (event) => {
  ...
  if (event.target.id === 'search-input') {
    ...
    if (search && state.searchQuery.length > 0) {
